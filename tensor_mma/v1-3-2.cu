...
#pragma unroll
        for(int S_A_row_id = S_A_row; S_A_row_id < BM; S_A_row_id += 256 / 4)//每行4个thread, 256个thread一次拿64行
        {
            ...
            (float4 &)S_A[(S_A_row_id) * (BK + padding_SA) + S_A_col] = (float4 &)dA[(S_A_row_id + row_offset) * K + S_A_col + base_k];
        }
        // ===== 加载 B 矩阵到共享内存 =====
...
#pragma unroll
            for(int i = 0; i < WARP_M / MMA_M; i++)
            {
                ...
                uint32_t A_smem_lane = __cvta_generic_to_shared(&S_A[(lane_id % 16 + i * MMA_M + S_AC_row_offset) * (BK + padding_SA) + (lane_id / 16) * 8 + split_MMA_K_id * MMA_K]);//加上偏移
