...
__global__ void matrixKernel(half *dA, half *dB, half *dC, int M, int K, int N)
{
    ...
    //double buffer所带来的偏移
    int buffer_offset = BM * S_A_STRIDE + BK * S_B_STRIDE;
    int buffer_offset_load = buffer_offset;
    int buffer_offset_store = 0;
    
    half* S_A = smem;
    half* S_B = smem + BM * S_A_STRIDE;
    ...
    // Registers for MMA
    //开double buffer
    int register_buffer_load = 1;
    int register_buffer_store = 1;
    uint32_t RA[2][4][4];  // 4 M-tiles (4 * 16) * 4 regs (16 words)
    uint32_t RB[2][8][2];  // 8 N-tiles (8 * 8) * 2 regs (8 words)
...
//第0轮取数
    ...
    // K维度循环
    for(split_k_id = 1; split_k_id < (K + BK - 1) / BK; split_k_id++)
    {
        buffer_offset_store ^= buffer_offset;
        buffer_offset_load ^= buffer_offset;
        S_A = smem + buffer_offset_load;
        S_B = smem + BM * S_A_STRIDE + buffer_offset_load;
...
//第 split_k_id 轮存数
        base_k = split_k_id * BK;
        S_A = smem + buffer_offset_store;
        S_B = smem + BM * S_A_STRIDE + buffer_offset_store;
        // dA -> S_A
        #pragma unroll
        for(int S_A_row_id = S_A_row; S_A_row_id < BM; S_A_row_id += 256 / 4)
        {
...
// 第 split_k_id轮 计算   
    ...
    for(int i = 0; i < WARP_M / MMA_M; i++)
    {
        ...
        for(int j = 0; j < WARP_N / MMA_N; j++)
        {
            ...
            int base_idx = (lane_id / 4 + i * MMA_M + S_AC_row_offset) * C_STRIDE + j * MMA_N + S_BC_col_offset;
    ...
    for(int S_C_row_id = S_C_row; S_C_row_id < BM; S_C_row_id += 256 / 16)
    {
        if (S_C_row_id + row_offset < M && S_C_col + col_offset < N) {
            ...
            int smem_idx = (S_C_row_id) * C_STRIDE + S_C_col;
            (float4 &)dC[(S_C_row_id + row_offset) * N + S_C_col + col_offset] = (float4 &)S_C[smem_idx];
        }
    }

    //方法2每个warp读的连续的
    // int row_offset_warp = row_offset + (BM / 8) * warp_id;//每个warp起始位置
    // int S_C_row = lane_id / 16;
    // int S_C_col = (lane_id % 16) * 8;
    // #pragma unroll
    // for(int S_C_row_id = S_C_row; S_C_row_id < BM / 8; S_C_row_id += 2)//每个warp一次拿两行
    // {
    //     if (S_C_row_id + row_offset_warp < M && S_C_col + col_offset < N) {
    //         // S_C 读取：使用原始 BN 步长 (128)
    //         int smem_idx = (S_C_row_id + (BM / 8) * warp_id) * C_STRIDE + S_C_col;
    //         (float4 &)dC[(S_C_row_id + row_offset_warp) * N + S_C_col + col_offset] = (float4 &)S_C[smem_idx];
    //     }
    // }
}
...
void hostMatrix(half *hostA, half *hostB, half *hostC, int M, int K, int N)
{
    ...
    size_t smem_size_ab_half = (BM * S_A_STRIDE + BK * S_B_STRIDE) * 2;
    size_t smem_size_c_half = BM * C_STRIDE;
