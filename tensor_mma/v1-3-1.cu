...
__global__ void matrixKernel(half *dA, half *dB, half *dC, int M, int K, int N)
{
    ...
    for(int split_k_id = 0; split_k_id < (K + BK - 1) / BK; split_k_id++)
    {
        ...
        for(int S_A_row_id = S_A_row; S_A_row_id < BM; S_A_row_id += 256 / 4)//每行4个thread, 256个thread一次拿64行
        {
            ...
            (float4 &)S_A[(S_A_row_id) * (BK + padding) + S_A_col] = (float4 &)dA[(S_A_row_id + row_offset) * K + S_A_col + base_k];
        }
        // ===== 加载 B 矩阵到共享内存 =====
        ...
        {
            (float4 &)S_B[(S_B_row_id) * (BN + padding) + S_B_col] = (float4 &)dB[(S_B_row_id + base_k) * N + S_B_col + col_offset];
        }
        __syncthreads();
        ...
        for(int split_MMA_K_id = 0; split_MMA_K_id < BK / MMA_K; split_MMA_K_id++)
        {
            ...
            for(int i = 0; i < WARP_M / MMA_M; i++)
            {
                ...
                uint32_t A_smem_lane = __cvta_generic_to_shared(&S_A[(lane_id % 16 + i * MMA_M + S_AC_row_offset) * (BK + padding) + (lane_id / 16) * 8 + split_MMA_K_id * MMA_K]);//加上偏移
        ...
        //S_B ->右矩阵，对S_B纵着切
            ...
            for(int j = 0; j < WARP_N / MMA_N; j++)
            {
                ...
                uint32_t B_smem_lane = __cvta_generic_to_shared(&S_B[(tid % 16 + split_MMA_K_id * MMA_K) * (BN + padding) + j * MMA_N + S_BC_col_offset]);
...
void hostMatrix(half *hostA, half *hostB, half *hostC, int M, int K, int N)
{
    ...
    //padding下的调用
    size_t smem_max_size = std::max((BM * (BK + padding) + BK * (BN + padding)) * sizeof(half), BM * BN * sizeof(half));
    cudaFuncSetAttribute(matrixKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size);
