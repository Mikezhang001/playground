...
#pragma unroll
        for(int S_B_row_id = S_B_row; S_B_row_id < BK; S_B_row_id += 256 / 16)//每行16个thread, 256个thread一次拿16行
        {
            ...
            //仔细观察会发现，每行都需要做偏移，以8为周期是因为后面每8个thread为一quarter_warp
            (float4 &)S_B[(S_B_row_id) * BN + ((S_B_col / 8 + S_B_row_id % 8) % (BN / 8)) * 8] = (float4 &)dB[(S_B_row_id + base_k) * N + S_B_col + col_offset];
        }
        __syncthreads();
...
#pragma unroll
            for(int j = 0; j < WARP_N / MMA_N; j++)
            {
                ...
                // uint32_t B_smem_lane = __cvta_generic_to_shared(&S_B[(tid % 16 + split_MMA_K_id * MMA_K) * BN + j * MMA_N + S_BC_col_offset]);

                //仔细观察会发现，每行都需要做偏移，以8为周期是因为后面每8个thread为一quarter_warp, 原来列j * MMA_N + S_BC_col_offset，偏移(tid % 16) % 8
                uint32_t B_smem_lane = __cvta_generic_to_shared(&S_B[(tid % 16 + split_MMA_K_id * MMA_K) * BN + (((j * MMA_N + S_BC_col_offset) / 8 + (tid % 16) % 8) % (BN / 8)) * 8]);
