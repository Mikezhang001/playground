...
__global__ void matrixKernel(half *dA, half *dB, half *dC, int M, int K, int N)
{
    ...
    __shared__ half S_A[BM * (BK)];//一维数组，且空间开为之前的两倍, 不加padding可以开double buffer。但去掉padding比v3慢太多，不觉得真实现double buffer速度会很快
    ...
    for(int split_k_id = 0; split_k_id < K / BK; split_k_id++)
    {
        ...
        S_A_addr0 = S_A_base_addr + ((S_A_row) * (BK ) + S_A_col * 8) * sizeof(half);
        S_A_addr1 = S_A_base_addr + ((S_A_row + 1) * (BK) + S_A_col * 8) * sizeof(half);
        S_B_addr0 = S_B_base_addr + ((S_B_row) * (BN ) + S_B_col * 8) * sizeof(half);
        S_B_addr1 = S_B_base_addr + ((S_B_row + 1) * (BN) + S_B_col * 8) * sizeof(half);
        S_B_addr2 = S_B_base_addr + ((S_B_row + 2) * (BN ) + S_B_col * 8) * sizeof(half);
        S_B_addr3 = S_B_base_addr + ((S_B_row + 3) * (BN ) + S_B_col * 8) * sizeof(half);

        //dA->S_A
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(S_A_addr0), "l"(&dA[(S_A_row + row_offset) * K + (S_A_col * 8 + split_k_id * BK)]));
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(S_A_addr1), "l"(&dA[(S_A_row + 1 + row_offset) * K + (S_A_col * 8 + split_k_id * BK)]));
        //dB->S_B
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(S_B_addr0), "l"(&dB[(S_B_row + split_k_id * BK) * N + (S_B_col * 8 + col_offset)]));
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(S_B_addr1), "l"(&dB[(S_B_row + 1 + split_k_id * BK) * N + (S_B_col * 8 + col_offset)]));
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(S_B_addr2), "l"(&dB[(S_B_row + 2 + split_k_id * BK) * N + (S_B_col * 8 + col_offset)]));
        asm ("cp.async.ca.shared.global [%0], [%1], 16;\n" :
            : "r"(S_B_addr3), "l"(&dB[(S_B_row + 3 + split_k_id * BK) * N + (S_B_col * 8 + col_offset)]));
        ...
        for(int row = 0; row < 64 / 16; row++)
        {
            ...
            {
                wmma::load_matrix_sync(left_frag[col][row], &S_A[(16 * row + 64 * warp_id_y) * (BK ) + 16 * col], BK);//改成一维度的寻址
            }
        }
        ...
        for(int row = 0; row < 32 / 16; row++)
        {
            ...
            {
                wmma::load_matrix_sync(right_frag[row][col], &S_B[(16 * row) * (BN ) + 16 * col + 64 * warp_id_x], BN);//改成一维度的寻址
            }
        }
