...
__global__ void matrixKernel7st(float *dA, float *dB, float *dC, int M, int K, int N)
{
    int row, col;
    row = (blockIdx.y * blockDim.y) * TM;
    col = (blockIdx.x * blockDim.x) * TN;

    __shared__ float S_A[blockDim_y][split_K];//想较于v3.cu split_K 扩大了4倍
    ...
    for(int split_K_id = 0; split_K_id < (K + split_K - 1) / split_K; split_K_id++)
    {
        ...
        //左边二维， 右边一维索引
        (float4 &)(S_A[(S_A_row)][S_A_col * 4]) = (float4 &)(dA[(S_A_row + row) * K + S_A_col * 4 + k_idx]);
...
PLAYGROUND_MATMUL_DEC(float32_t, 7, M, N, K, A, B, C)
{
    ...
    matrixKernel7st<SPLIT_K, LOGIC_BLOCK_DIM_Y, LOGIC_BLOCK_DIM_X, TM, TN><<<grid_dim, block_dim>>>(const_cast<float*>(A), const_cast<float*>(B), const_cast<float*>(C), M, K, N);
