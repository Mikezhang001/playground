...
__global__ void matrixKernel10st(float *dA, float *dB, float *dC, int M, int K, int N)
{
    int row, col;
...
PLAYGROUND_MATMUL_DEC(float32_t, 10, M, N, K, A, B, C)
{
    ...
    matrixKernel10st<SPLIT_K, LOGIC_BLOCK_DIM_Y, LOGIC_BLOCK_DIM_X, TM, TN><<<grid_dim, block_dim>>>(const_cast<float*>(A), const_cast<float*>(B), const_cast<float*>(C), M, K, N);
