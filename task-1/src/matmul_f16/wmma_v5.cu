...
__global__ void matrixKernel11s(half *dA, half *dB, half *dC, int M, int K, int N)
{
...
PLAYGROUND_MATMUL_DEC(float16_t, 11, M, N, K, A, B, C)
{
    ...
    matrixKernel11s<BM, BK, BN><<<grid_dim, block_dim>>>(const_cast<float16_t*>(A), const_cast<float16_t*>(B), const_cast<float16_t*>(C), M, K, N);
