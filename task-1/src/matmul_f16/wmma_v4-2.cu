...
__global__ void matrixKernel6s(half *dA, half *dB, half *dC, int M, int K, int N)
{
    ...
    {
        //dA->S_A
        for(int S_A_col_id = S_A_col; S_A_col_id < BK / 8; S_A_col_id += (256 / 128))//一个float4可以取8个half
        {
            *reinterpret_cast<float4*>(&(S_A[S_A_row][S_A_col_id * 8])) = *reinterpret_cast<float4*>(&(dA[(S_A_row + row_offset) * K + (S_A_col_id * 8 + split_k_id * BK)]));
        }

        //dB->S_B
        for(int S_B_col_id = S_B_col; S_B_col_id < BN / 8; S_B_col_id += (256 / 32))//一个float4可以取8个half
        {
            *reinterpret_cast<float4*>(&(S_B[S_B_row][S_B_col_id * 8])) = *reinterpret_cast<float4*>(&(dB[(S_B_row + split_k_id * BK) * N + (S_B_col_id * 8 + col_offset)]));
        }
        __syncthreads();
...
PLAYGROUND_MATMUL_DEC(float16_t, 6, M, N, K, A, B, C)
{
    ...
    matrixKernel6s<BM, BK, BN><<<grid_dim, block_dim>>>(const_cast<float16_t*>(A), const_cast<float16_t*>(B), const_cast<float16_t*>(C), M, K, N);
