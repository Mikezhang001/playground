...
__global__ void matrixKernel12s(half *dA, half *dB, half *dC, int M, int K, int N)
{
...
PLAYGROUND_MATMUL_DEC(float16_t, 12, M, N, K, A, B, C)
{
    ...
    cudaFuncSetAttribute(matrixKernel12s, cudaFuncAttributeMaxDynamicSharedMemorySize, 98304);//只能传不带模板的进去, 96KB
    unsigned int dsmem = 2 * (BM * (BK + 8) + BK * (BN + 8)) * sizeof(half);
    matrixKernel12s<<<grid_dim, block_dim, dsmem>>>(const_cast<float16_t*>(A), const_cast<float16_t*>(B), const_cast<float16_t*>(C), M, K, N);
