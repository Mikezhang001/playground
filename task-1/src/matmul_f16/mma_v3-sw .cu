...
__global__ void matrixKernel21s(half *A, half *B, half *C, int M, int K, int N)
{
...
PLAYGROUND_MATMUL_DEC(float16_t, 21, M, N, K, A, B, C)
{
    ...
    cudaFuncSetAttribute(matrixKernel21s, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size);
    
    matrixKernel21s<<<grid_dim, block_dim, smem_max_size>>>(
