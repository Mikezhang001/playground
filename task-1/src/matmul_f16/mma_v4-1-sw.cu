...
__global__ void matrixKernel24s(half *A, half *B, half *C, int M, int K, int N)
{
...
#pragma unroll
    ...
    //----------------------------第0轮 global -> shared memory

    S_A = &smem[0][0];
    S_B = &smem[0][0] + BM * BK;
    int base_k = 0 * CHUNK_K * MMA_K;
    ...
    base_k = CHUNK_K * MMA_K;

    dst_1 = __cvta_generic_to_shared(&S_A[(S_A_row) * BK + (S_A_col / 8 + (S_A_row % 8) / 2) % 4 * 8]);
    src_1 = (uint64_t)A + (uint64_t)((S_A_row + row_offset) * K + S_A_col + base_k) * sizeof(half);
    asm volatile("cp.async.cg.shared.global [%0], [%1], %2;\n" ::"r"(dst_1), "l"(src_1), "n"(16));
    
    dst_2 = __cvta_generic_to_shared(&S_A[(S_A_row + 8) * BK + (S_A_col / 8 + ((S_A_row + 8) % 8) / 2) % 4 * 8]);
    src_2 = (uint64_t)A + (uint64_t)((S_A_row + 8 + row_offset) * K + S_A_col + base_k) * sizeof(half);
    asm volatile("cp.async.cg.shared.global [%0], [%1], %2;\n" ::"r"(dst_2), "l"(src_2), "n"(16));

    dst_3 = __cvta_generic_to_shared(&S_A[(S_A_row + 16) * BK + (S_A_col / 8 + ((S_A_row + 16) % 8) / 2) % 4 * 8]);
    src_3 = (uint64_t)A + (uint64_t)((S_A_row + 16 + row_offset) * K + S_A_col + base_k) * sizeof(half);
    asm volatile("cp.async.cg.shared.global [%0], [%1], %2;\n" ::"r"(dst_3), "l"(src_3), "n"(16));

    dst_4 = __cvta_generic_to_shared(&S_A[(S_A_row + 24) * BK + (S_A_col / 8 + ((S_A_row + 24) % 8) / 2) % 4 * 8]);
    src_4 = (uint64_t)A + (uint64_t)((S_A_row + 24 + row_offset) * K + S_A_col + base_k) * sizeof(half);
    asm volatile("cp.async.cg.shared.global [%0], [%1], %2;\n" ::"r"(dst_4), "l"(src_4), "n"(16));


    dst_5 = __cvta_generic_to_shared(&S_B[(S_B_row) * (BN + padding_SB) + S_B_col]);
    src_5 = (uint64_t)B + (uint64_t)((S_B_row + base_k) * N + S_B_col + col_offset) * sizeof(half);
    asm volatile("cp.async.cg.shared.global [%0], [%1], %2;\n" ::"r"(dst_5), "l"(src_5), "n"(16));

    dst_6 = __cvta_generic_to_shared(&S_B[(S_B_row + 2) * (BN + padding_SB) + S_B_col]);
    src_6 = (uint64_t)B + (uint64_t)((S_B_row + 2 + base_k) * N + S_B_col + col_offset) * sizeof(half);
    asm volatile("cp.async.cg.shared.global [%0], [%1], %2;\n" ::"r"(dst_6), "l"(src_6), "n"(16));
...
//----------------------------第0轮 第0个 shared memory -> register 
    ...
    // buffer_offset_load_idx = (buffer_offset_load_idx + 1) % K_STAGE;
    // buffer_offset_load = buffer_offset_load_idx * BUFFER_OFFSET;//初始为0不需要偏移

    S_A = &smem[0][0] + buffer_offset_load;
    S_B = &smem[0][0] + BM * BK + buffer_offset_load;
    //寄存器侧展开
...
// //----------------------------循环开始
#pragma unroll
    for (size_t tile_k = (K_STAGE - 1) * CHUNK_K; tile_k < K_tiles; tile_k += CHUNK_K) {
...
#pragma unroll
        for (size_t i = 0; i < WARP_COL_TILES; ++i) {
            ...
            // uint32_t A_smem_lane_addr = __cvta_generic_to_shared( &smem[(A_smem_idx + lane_id % 16)][((lane_id / 16 + k_step * MMA_K / 8 + (lane_id % 8) / 2)) % 4 * 8]);//
            uint32_t A_smem_lane_addr = __cvta_generic_to_shared(&S_A[(A_smem_idx + lane_id % 16) * BK + ((lane_id / 16 + ((k_step) % CHUNK_K) * MMA_K / 8 + (lane_id % 8) / 2)) % 4 * 8]);//                  
            asm volatile("ldmatrix.sync.aligned.x4.m8n8.shared.b16 {%0, %1, %2, %3}, [%4];\n" \
                : "=r"(RA[reg_store_idx][i][0]), "=r"(RA[reg_store_idx][i][1]), "=r"(RA[reg_store_idx][i][2]), "=r"(RA[reg_store_idx][i][3])                     \
                : "r"(A_smem_lane_addr));    
...
#pragma unroll
        for(int j = 0; j < WARP_ROW_TILES; j++)
        {
            ...
            uint32_t B_smem_lane = __cvta_generic_to_shared(&S_B[(tid % 16 + ((k_step) % CHUNK_K) * MMA_K) * (BN + padding_SB) + j * MMA_N + S_BC_col_offset]);
            asm volatile("ldmatrix.sync.aligned.x2.trans.m8n8.shared.b16 {%0, %1}, [%2];\n" \
                : "=r"(RB[reg_store_idx][j][0]), "=r"(RB[reg_store_idx][j][1])                                   \
                : "r"(B_smem_lane));//r 四个字节，l 八个字节    
...
PLAYGROUND_MATMUL_DEC(float16_t, 24, M, N, K, A, B, C)
{
    ...
    // int buffer_num = 2;
    size_t smem_max_size = std::max((BM * BK + BN * (BK +  padding_SB)) * sizeof(half) * K_STAGE, BM * BN * sizeof(half));
    cudaFuncSetAttribute(matrixKernel24s, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size);
    
    matrixKernel24s<<<grid_dim, block_dim, smem_max_size>>>(
