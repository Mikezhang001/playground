...
__global__ void matrixKernel3st(half *dA, half *dB, half *dC, int M, int K, int N)
{
    ...
    //shared memory部分
    __shared__ half S_A[(WARP_DIM_Y * WARP_DIM_X) * (WMMA_M * WMMA_K)];
    __shared__ half S_B[(WARP_DIM_Y * WARP_DIM_X) * (WMMA_K * WMMA_N)];
    ...
    for(int split_k_id = 0; split_k_id < (K + WMMA_K - 1)/(WMMA_K); split_k_id++)
    {
        ...
        {
            S_A[S_A_row * WMMA_K + S_A_col_id + warp_id * WMMA_M * WMMA_K] = dA[(row_offset + S_A_row) * K + (split_k_id * WMMA_K) + S_A_col_id];
        }

        //dB->S_B
        for(int S_B_row_id = S_B_row; S_B_row_id < WMMA_K; S_B_row_id += (WARP_SIZE / WMMA_N))//一次拿WARP_SIZE / WMMA_N行
        {
            S_B[S_B_row_id * WMMA_N + S_B_col + warp_id * WMMA_K * WMMA_N] = dB[(split_k_id * WMMA_K + S_B_row_id) * N + col_offset + S_B_col];
        }
        __syncthreads();

        wmma::load_matrix_sync(left_frag, S_A + warp_id * WMMA_M * WMMA_K, WMMA_K);
        wmma::load_matrix_sync(right_frag, S_B + warp_id * WMMA_K * WMMA_N, WMMA_N);
...
PLAYGROUND_MATMUL_DEC(float16_t, 3, M, N, K, A, B, C)
{
    ...
    matrixKernel3st<WMMA_M, WMMA_N, WMMA_K, WARP_SIZE, WARP_DIM_X, WARP_DIM_Y><<<grid_dim, block_dim>>>(const_cast<float16_t*>(A), const_cast<float16_t*>(B), const_cast<float16_t*>(C), M, K, N);
