...
__global__ void matrixKernel20s(half *A, half *B, half *C, int M, int K, int N)
{
...
PLAYGROUND_MATMUL_DEC(float16_t, 20, M, N, K, A, B, C)
{
    ...
    cudaFuncSetAttribute(matrixKernel20s, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size);
    
    matrixKernel20s<<<grid_dim, block_dim, smem_max_size>>>(
