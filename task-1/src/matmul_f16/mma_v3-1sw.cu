...
__global__ void matrixKernel22s(half *A, half *B, half *C, int M, int K, int N)
{
...
PLAYGROUND_MATMUL_DEC(float16_t, 22, M, N, K, A, B, C)
{
    ...
    cudaFuncSetAttribute(matrixKernel22s, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size);
    
    matrixKernel22s<<<grid_dim, block_dim, smem_max_size>>>(
