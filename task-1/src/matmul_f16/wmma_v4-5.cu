...
__global__ void matrixKernel9s(half *dA, half *dB, half *dC, int M, int K, int N)
{
    ...
    for(int split_k_id = 0; split_k_id < K / BK; split_k_id++)
    {
        ...
        FLOAT4(S_A[S_A_row + 64][S_A_col * 8]) = FLOAT4(dA[(S_A_row + 64 + row_offset) * K + (S_A_col * 8 + split_k_id * BK)]);
        //dB->S_B
        FLOAT4(S_B[S_B_row     ][S_B_col * 8]) = FLOAT4(dB[(S_B_row + split_k_id * BK) * N + (S_B_col * 8 + col_offset)]);
        FLOAT4(S_B[S_B_row + 8 ][S_B_col * 8]) = FLOAT4(dB[(S_B_row + 8 + split_k_id * BK) * N + (S_B_col * 8 + col_offset)]);
        FLOAT4(S_B[S_B_row + 16][S_B_col * 8]) = FLOAT4(dB[(S_B_row + 16 + split_k_id * BK) * N + (S_B_col * 8 + col_offset)]);
        FLOAT4(S_B[S_B_row + 24][S_B_col * 8]) = FLOAT4(dB[(S_B_row + 24 + split_k_id * BK) * N + (S_B_col * 8 + col_offset)]);
...
PLAYGROUND_MATMUL_DEC(float16_t, 9, M, N, K, A, B, C)
{
    ...
    matrixKernel9s<BM, BK, BN><<<grid_dim, block_dim>>>(const_cast<float16_t*>(A), const_cast<float16_t*>(B), const_cast<float16_t*>(C), M, K, N);
