...
__global__ void matrixKernel4s(half *dA, half *dB, half *dC, int M, int K, int N)
{
    ...
    {
        //dA->S_A
        for(int S_A_col_id = S_A_col; S_A_col_id < BK; S_A_col_id += (256 / 128))
        {
            S_A[S_A_row][S_A_col_id] = dA[(S_A_row + row_offset) * K + (S_A_col_id + split_k_id * BK)];
        }
        //dB->S_B
        for(int S_B_row_id = S_B_row; S_B_row_id < BK; S_B_row_id += (256 / 256))
        {
            S_B[S_B_row_id][S_B_col] = dB[(S_B_row_id + split_k_id * BK) * N + (S_B_col + col_offset)];
        }
        __syncthreads();
        ...
        for(int row = 0; row < 64 / 16; row++)
        {
            ...
            {
                wmma::load_matrix_sync(left_frag[row][col], &S_A[16 * row + 64 * warp_id_y][16 * col], BK);
            }
        }
        ...
        for(int row = 0; row < 32 / 16; row++)
        {
            ...
            {
                wmma::load_matrix_sync(right_frag[row][col], &S_B[16 * row][16 * col + 64 * warp_id_x], BN);
            }
        }
        __syncthreads();
...
PLAYGROUND_MATMUL_DEC(float16_t, 4, M, N, K, A, B, C)
{
    ...
    matrixKernel4s<BM, BK, BN><<<grid_dim, block_dim>>>(const_cast<float16_t*>(A), const_cast<float16_t*>(B), const_cast<float16_t*>(C), M, K, N);
